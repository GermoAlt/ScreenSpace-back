...
public class MovieController implements MoviesApi {
    ...
    @Override
    public ResponseEntity<List<MoviesPerCinema>> searchMovies(@Valid String cinema, @Valid String movieTitle, @Valid String genre, @Valid String score, @Valid String latitute, @Valid String longitude) {
        var groupedByCinema = screeningService.searchScreenings(cinema, movieTitle, genre, score, latitute, longitude)
                .stream()
        ...
        return ResponseEntity.ok(groupedByCinema.entrySet().stream().map(f -> {
            ...
            return new MoviesPerCinema()
                ...
                .distance(latitute != null && longitude != null ? Double.toString(c.calculateDistanceToCinema(Double.parseDouble(latitute), Double.parseDouble(longitude))) : null);
